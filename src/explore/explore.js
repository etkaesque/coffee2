...
function filterCoffee(coffees) {
  ...
  form.addEventListener("submit", (event) => {
    ...
    function getCheckedBeans() {
      const checkedBeans = document.querySelectorAll(
        '#filterForm input[name="beans"]:checked'
      );
      const checkedBeanValues = Array.from(checkedBeans).map(
        (input) => input.value
      );
    ...
    function getCheckedRegions() {
      const checkedRegions = document.querySelectorAll(
        '#filterForm input[name="regions"]:checked'
      );
      const checkedRegionsValues = Array.from(checkedRegions).map(
        (input) => input.value
      );
    ...
    let filteredArray = coffees.filter((obj) => {
      if (!userRoast) {
        if (
          obj.altitude.minAltitude >= userAltitude &&
          userBeans.includes(obj.beans) &&
